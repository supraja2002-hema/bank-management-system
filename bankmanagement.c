...
void view_list()
{
	...
	{
		printf("\n%6d\t %20s\t\t\t%30s\t\t%.0lf",add.acc_no,add.name,add.address,add.phone);
		test++;
	}
	fclose(view);
	if(test==0)
...
void erase(void)
{
	...
	if(test=0)
	{
		printf("\nRecord not found\a\a\a");
		erase_invalid:
			printf("\nEnter 0 to try again,1 to return to main menu and 0 to exit");
			scanf("%d",&main_exit);
			if(main_exit==1)
			menu();
			else
...
void see(void)
{
    ...
    {   
        ...
        scanf("%d",&check.acc_no);
        while((fscanf(ptr,"%d%s%d/%d/%d%d%s%lf%s%f%d/%d/%d",&add.acc_no,add.name,&add.dob.month,&add.dob.day,&add.dob.year,&add.age,add.address,&add.phone,add.acc_type,&add.amt,&add.deposit.day,&add.deposit.month,&add.deposit.year)!=EOF))
        {
...
void close(void)
{
	printf("\n\nThis C project is developed by Supraja");
}
void menu(void)
{
	int choice;
...
int main()
{
	char pass[10],password[10]="captain";
	int i=0;
	...
	if(strcmp(pass,password)==0)
    {
    	...
    	for(i=0;i<=6;i++)
    	{
    		fordelay(1000);//delays for 1 second
    		printf(".");
		}
		system("cls");
		menu();
